...
/*
void
CirMgr::getToken(const string& option, vector<string>& options)
{
    string token;
    string second = option;
    myStrGetTok(option, token);
    options.push_back(token);
    second = second.substr(options.size() +1);
    options.push_back(second);
}
*/
void
...
//        return false;
   ...
   // for(size_t i = 0 ; i <6 ; i++)
    ...
    for(unsigned i = 0 ; i < header[4] ; i++)
    {
      ...
      //  cout<< gate2 -> getOutputGate(0) -> getId() << endl;
    }
    //construct output gate PO
    ...
    //define name of gates and tailing comments
...
{
    vector<string> aig;
 //   vector<unsigned> faninaig1;
 //   vector<unsigned> faninaig2;
    ...
    //print out
    outfile << "aag " << header[0] <<" "<< header[1] <<" "<< header[2] <<" "<<header[3]
            << " " << header[4] <<endl;
    //pi
    for(unsigned i = 0 ; i < ingate.size(); i++)
