...
TaskMgr::add(size_t nMachines)
{
   // TODO...
    ...
    {
        TaskNode _node;
        _taskHash.insert(_node);
        _taskHeap.insert(_node);
        cout<<"Task node inserted: " << _node <<endl;
    }
}
...
TaskMgr::assign(size_t l)
{
   ...
   if(_taskHeap.size() == 0)
       return false;
   TaskNode _node(min().getName(), min().getLoad() + l);
   _taskHash.update(_node);
   _taskHeap.delMin();
   _taskHeap.insert(_node);
   return true;
}
