...
CmdParser::openDofile(const string& dof)
{
   ...
   _dofileStack.push(_dofile);	//else:stack in one dofile
   return true;
}
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
   string cmd;
   size_t endptr = myStrGetTok(str ,cmd);	//get the first word
   if(cmd != "")
   {
	size_t ptr = str.find_first_not_of(' ',endptr);
	if(ptr == string::npos)			//return trailiing part
		option ="";
	else
		option = str.substr(ptr);
	if(getCmd(cmd) == 0)	//return error 
...
{
   // TODO...
    //to cut all the blank in the front
    string token;
    myStrGetTok(str,token);

    size_t beginptr = str.find_first_not_of(' ',0);
    string newstring;
    if(beginptr!=string::npos)
        newstring = str.substr(beginptr);
    
    //case1: NULL string->print all
    if(token == "")
    {
    int count =0;
    cout<<endl;
    ...
    {
    vector<CmdMap::const_iterator> tmp;
    vector<string> tmpstr;
    for(CmdMap::const_iterator it = _cmdMap.begin();it !=_cmdMap.end();it++)
    {
        string temp = it->first+it->second->getOptCmd();
        //fully matched
        if(myStrNCmp(temp,token,it->first.size()) == 0)
        {
            tmp.push_back(it);
            tmpstr.push_back(temp);
        }
        //partially matched
        else if(newstring.size() < it->first.size())
        {
            if(myStrNCmp(temp,newstring,newstring.size()) == 0)
            {
                tmp.push_back(it);
                tmpstr.push_back(temp);
            }
    ...
    //case2: partially matched(multiple choices)
    if(tmp.size() >1)
    {
        cout<<endl;
        int count =0;
    ...
    //case3: single match (partially) && case 5: already matched
    else if(tmp.size() == 1)
    {
        // already matched can be multiple words ->case 5
        if(newstring.size() > token.size())
        {
            cout<<endl;
            tmp[0]->second->usage(cout);
            reprintCmd();
        }
        // partially matched must only be one word ->case 3
        else
        {
            for(size_t i = newstring.size(); i<tmpstr[0].size();i++)
                insertChar(tmpstr[0][i]);
        }
    }
    //case4: no matches && case6:no matches
    else if(tmp.empty())
    {
        mybeep();
    }
...
CmdParser::getCmd(string cmd)
{
   ...
   {  
	if(myStrNCmp(iter -> first + iter -> second -> getOptCmd(), cmd, iter ->first.size()) == 0)
        { 
		e = iter -> second;
		break;
        }
