...
DBAppendCmd::exec(const string& option)
{
   ...
   // check option
   int care;
   //check if dbtbl exsists
   if(checkRowIdx("0",care))
    	return CMD_EXEC_DONE;
   vector<string> tokens;
   CmdExec::lexOptions(option,tokens);	
   if(tokens.empty())           //default:if not get ant tokens
	    tokens.push_back("");
   //check if the first word is valid(only -row and -col)
   if(myStrNCmp("-Column",tokens[0],2) != 0)
   ...
   //to store data in tokens(str to int)
   vector <int> value;
   for(size_t i = 1; i < tokens.size(); i++)
   {
        ...
        if(tokens[i] == "-")
            temp = INT_MAX;
        else if(myStr2Int(tokens[i], temp) != 0)    //not an int
        {
   ...
   //to append the right data into table dbtbl
   if(myStrNCmp("-Column",tokens[0],2 == 0))    //if append col
   {
        while(value.size() < dbtbl.nRows())           //if not long enough
            value.push_back(INT_MAX);
        dbtbl.addCol(value);
   }
   else if(myStrNCmp("-Row",tokens[0],2) == 0)  //if append row
   {
        while(value.size() < dbtbl.nCols())           //if not long enough
            value.push_back(INT_MAX);
        dbtbl.addRow(DBRow(value));          
   }
   return CMD_EXEC_DONE;
}
...
{  
   // TODO..
    //check if dbtbl exsists
    int care;
    if(checkRowIdx("0",care)!= 0)
        return CMD_EXEC_DONE;
    //cut the string
    vector<string> tokens;
    CmdExec::lexOptions(option,tokens);
    
    //case 1: print out a row
    if(myStrNCmp("-Row",tokens[0],2) == 0)
    {
        int row;
        bool check = checkRowIdx (tokens[1],row); 
        if(tokens.size() < 2)
            return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
        else if(tokens.size() >2)
            return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[0]);
        
        if(check ==  0)
            return CMD_EXEC_ERROR;
        else if(check == 1) //finally survived all the test above ^^
            cout<<dbtbl[row];
    }
    //case 2: print out a column
    else if(myStrNCmp("-Column",tokens[0],2) ==0)
    {
        int col;
        bool check = checkColIdx(tokens[1],col);
        if(tokens.size() <2)
            return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
        else if(tokens.size() > 2)
            return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[0]);
        ...
        if(check ==0)
            return CMD_EXEC_ERROR;
        else if(check ==1)//finally survived ^^
            dbtbl.printCol(col);
    }
    //case 3:print out a table
    ...
    //case 5:print out cell
    else if(tokens.size() == 2)
    {
        int row,col;
        bool checkrow = checkRowIdx(tokens[0], row);
        bool checkcol = checkColIdx(tokens[1], col);
        if(checkrow == 0 || checkcol == 0)
            return CMD_EXEC_ERROR;
        else
        {
            dbtbl.printData(cout, dbtbl.getData(row,col),true);
            cout<<endl;
        }
    }
    //case default
